...
def train(config: DictConfig):
    ...
    print(f"End of sentence toekn ID: ", end_of_sentence_token_id)
    illegal_token_mask = torch.zeros(tokenizer.vocab_size, dtype=torch.bool)
    illegal_tokens_orig = OmegaConf.to_container(config.task.constraints.illegal_tokens)
    illegal_tokens = [
        [t] if isinstance(t, int) else tokenizer.encode(t, add_special_tokens=False)
        for t in illegal_tokens_orig
    ]
    # for t in illegal_tokens:

    #     print(t)
    for i in range(len(illegal_tokens_orig)):
        print(illegal_tokens_orig[i])
    ...
    teacher = get_teacher(config, end_of_sentence_token_id, illegal_token_mask)
...
def get_model(config: DictConfig):
    ...
    model = AutoModelForCausalLM.from_pretrained(
            "/n/fs/nlp-jiatongy/extrapolate-positional-emb/ft_results/tokenizer-shard0/checkpoint-4500",
            # device_map = "auto",
...
def get_teacher(config: DictConfig, sentence_token_id, illegal_token_mask):
